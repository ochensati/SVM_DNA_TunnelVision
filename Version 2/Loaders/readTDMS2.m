function [ CurrentSignal, VoltageSignal, sampleInt, numPoints,alpha, maxVal, numchans] = readTDMS2(pathname, fn)  
...
%                         data = int16(zeros(chunkSize,1));
                  ...
                  %  maxVal(j) = max(maxVal(j),max(data));
                    if numchans == 1
                        CurrentSignal=[CurrentSignal data];
                    else
                        if j == 1
                            %fwrite(fidCur, data, 'int16');
                            CurrentSignal=[CurrentSignal data];
                        else
                            VoltageSignal=[VoltageSignal data];
...
CurrentSignal = cast( CurrentSignal,'double') .* -0.00030517578125;%alpha;
