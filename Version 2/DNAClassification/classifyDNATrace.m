...
%     emptyTrace = shortData( shortData>1.8)';
    trace = shortData - smooth(shortData,8000);
    trace2 = trace -mean(trace);%- smooth(trace, 30*1024);
    
    %turn the trace into a kmeans classified set of chunks
    % silhouette is used to optimize the step size of the traces
    step=150;
     [trace,trace2,trace3,colNames2,score,ident,bestSIL,infos]=  ParamAndOptimize(emptyTrace,trace,trace2,step,1,step,5,7);
...
%     cut=25;
    ...
    sampled = ident(1:floor(nSampled/step));
    cc=1;
    X=(1:length(trace3));
    ...
    state(floor(2.47e5/step):floor(2.594e5/step))=2;
    state(floor(2.758e5/step):floor(2.839e5/step))=2;
     state(floor(2.935e5/step):floor(3.005e5/step))=2;
       state(floor(3.272e5/step):floor(3.404e5/step))=2;
         state(floor(3.979e5/step):floor(4.084e5/step))=2;
%       state(floor(1.64e5/step):floor(1.75e5/step))=2;
%        state(floor(1.8e5/step):floor(1.95e5/step))=2;
...
if (true)
    ...
    for I=1:cState
        sI = idxS( idxI==I)* step;
        cuts{I}.trace = trace(sI(1):(sI(end)+step));
    end
    ...
    figure(2)
    for I=2:(length(starts)-2)
        
        template = traces(starts(I):ends(I));
        
        [r,l]=xcorr(traces,template);
        r=r(floor(find(l==0) - length(template)/2):end)/max(r);
     figure(2);   clf
        plot(r(starts(I):ends(I)))
        hold all
         plot(smooth(traces(starts(I):ends(I)),300)/max(traces))
        drawnow
        mm(I)=0;
        l=length(template)/2;
        for J=2:length(newTraces)-2
            mid=(ends(J)-starts(J))/2;
            sI=max([1 mid-500]);
            
            [m,i]=max(r(sI:(sI+1000)));
            m=m^.3;
         
            newTraces{J}.Ratings(I)=m;
            newTraces{J}.Pos(I)=500 - i;
            
            if (J==2)
                newTraces{I}.Ave = template;
            end
            mI=floor((starts(J)+ends(J))/2+ newTraces{J}.Pos(I)-l);
            if (mI>0)
                t=traces(mI:mI+length(template)-1);
                newTraces{I}.Ave = newTraces{I}.Ave + t*m;
            end
            if (J~=I)
                mm(I)=mm(I)+m;
            end
