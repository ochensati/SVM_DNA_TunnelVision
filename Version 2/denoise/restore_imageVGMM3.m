...
function [src,dst, levels, classes] = restore_imageVGMM3(src, covar, window, max_diff, weight_diff, iterations)


[src,levels]=Removeback(src,4);
t.m=mean(src);
t.s=std(src);
levels{1}=t;
...
for i = 1 : iterations
    ...
    if i<iterations
        ...
        idx = find(dmu<.001);%.5 previous to vgmm7
