function TestOnOrdered(conn,analyteNames,analytes,reducedData, SVMParams, runParams, colNames,parameterSet_Index)
...
trainIndex= fix(length(Labels)*.25);
orderedIDX =1:length(Labels);
orderedIDX(idx(1:trainIndex))=[];

Training = reducedData(idx(1:trainIndex),runParams.dataColStart:end);
LabelsTraining = Labels(idx(1:trainIndex));

Testing = reducedData(orderedIDX,runParams.dataColStart:end);
LabelsTesting = Labels(orderedIDX);
