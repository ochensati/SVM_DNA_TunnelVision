function [ extraInfo]= RandomSearch(superIteration, experimentName, reorganizedGroups, runParams,SVMParams ,extraInfo )
%RANDOMSEARCH Randomly searchs through the parameters to see what gives the
%best accuracy
    ...
    try
        if (strcmp( runParams.SVM_Method, 'Random')==1)
            ...
            if R<2
                switch (R)
                    case 0
                        nParams =randi( length(peakOnly)-1 )+1;
                        colNumbers = peakOnly( randperm(length(peakOnly),nParams) );
                    case 1
                        nParams =randi( length(clusterOnly)-1 )+1;
                        colNumbers = clusterOnly( randperm(length(clusterOnly),nParams) );
                    case 2
                        nParams =2;
            ...
            for J=1:nParams
                for K=1:length(allColNames)
                    if (strcmp( rSVMParameters{J}, allColNames{K})==true)
                        colNumbers =[colNumbers K]; %#ok<AGROW>
                    end
                end
            end
        end
