function [filteredData,lostPoints,lostPercent,lostDictionary] = FilterData(conn,experiment_Index,parameterSet_Index,analytes,analyteNames,reducedData, anomalySVM, commonSVM,runParams )
...
for K=1:length(analytes)
    ...
    for I=1:500:size(idx,1)
        ...
        top = min([ length(idx) I+499]);
        i2=(I:top);
        outs =1+ zeros(size(i2));
        temp= testData(i2,runParams.dataColStart:end);
        ABad=0;
        CBad=0;
        ...
        allBads = allBads + abs(length(i3) -length(i2));
        
        localGoods=localGoods + length(i3);
        localBads = localBads + abs(length(i3) -length(i2));
        
        
        if (goods >runParams.maxSVMPoints*2 && parameterSet_Index>0)
            break;
        end
    end
    
    
    if (length(idx)-length(goodIndexs))~=0
         disp(length(idx)-length(goodIndexs));    
    end
    filtered{K} = testData(goodIndexs,:); %#ok<AGROW>
    allIDX=allIDX+length(goodIndexs);
    %conn,experiment_Index,
    if (parameterSet_Index>0)
    ...
    lostDictionary{K,1}=analyteNames{K,1};
    lostDictionary{K,2}=localBads;
    lostDictionary{K,3}=localBads /(localGoods + localBads)*100;
end
