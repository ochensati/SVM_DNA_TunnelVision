function [ypred,vote,kernalValues]=svmmultivaloneagainstone(x,xsup,w,b,nbsv,kernel,kerneloption)
...
nbclass=floor((1+ sqrt(1+4*2*length(nbsv)))/2);
vote=zeros(n1,(nbclass));
kernalValues =zeros(n1,nbclass);
nbsv=[0 nbsv];
aux=cumsum(nbsv);
k=1;
for i=1:nbclass-1;
    for j=i+1:nbclass;
        ...
        kernalValues(indi,i)=kernalValues(indi,i)+ypred(indi);
        kernalValues(indj,j)=kernalValues(indj,j)-ypred(indj);
